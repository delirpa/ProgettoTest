...
{
	new Gallery_Meta_Box('gallery-manager', 'Gallery Manager', 'gallery');
}*/
...
function mix_gallery_callback()
{
	...
	switch ($action) {
		case 'image_delete':
			...
			break;
		case 'get_images':
			$attachments = mix_get_gallery((int)$_POST['id']);
			foreach ($attachments as $attach) {
			
				$photoTitle  = get_post_meta($_POST['id'], "att_attr_title_".$attach->ID, true);
				$photoCaption  = get_post_meta($_POST['id'], "att_attr_caption_".$attach->ID, true);
...
if (isset($_POST['save_new_title'])) {
	...
	$galleryItemTitle = nocarret(strip_tags($_POST['galleryItemTitle']));
	$galleryItemCaption = nocarret(strip_tags($_POST['galleryItemCaption']));
